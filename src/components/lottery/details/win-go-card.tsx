...
type LotteryWinGoDetailsCardProps = {
  details: {
    id: number;
    amount: number;
    profit: number | null;
...
export default function LotteryWinGoDetailsCard(props: LotteryWinGoDetailsCardProps) {
  ...
  const size = LOTTERY_SIZES.find((item) =>
    parseInt(number.value) > 4 ? item.name === "Big" : item.name === "Small",
  )!;
  ...
  return (
    <LotteryDetailsCard details={props.details} status={status as never}>
      <div className="flex justify-between gap-2.5 rounded-lg bg-white p-2.5 text-[0.625rem] leading-4">
        ...
        {props.details.profit !== null && props.details.lottery?.result && (
          <div className="flex items-center gap-2.5">
            ...
            <p className={twMerge("rounded", size.text)}>{t(size.name)}</p>
