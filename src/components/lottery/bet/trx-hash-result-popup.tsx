...
import { getTrxHashHistory, getTrxHashWinners } from "@data/lottery/trx-hash";
import { UnauthorizedError } from "@lib/error";
import { getLotteryWonLost } from "@lib/lottery";
import { getTranslations } from "@lib/translation";
import { rem } from "@lib/utils";

type LotteryTrxHashResultPopupProps = {
  type: string;
};

export default async function LotteryTrxHashResultPopup(props: LotteryTrxHashResultPopupProps) {
  const t = await getTranslations();
  const [winners, history] = await Promise.all([
    //
    getTrxHashWinners(props.type, 1).catch((error: unknown) => {
    ...
    getTrxHashHistory(1, { type: props.type }).catch((error: unknown) => {
      if (error instanceof UnauthorizedError) {
        return null;
      }
  ...
  if (number && won > 0) {
    return (
      <LotteryBetWinResult amount={won} id={winner.id} key={winner.id} prefix="trx-hash">
  ...
  if (number && lost > 0) {
    return (
      <LotteryBetLoseResult id={winner.id} key={winner.id} prefix="trx-hash">
        <p className="rounded bg-white px-4 py-1 text-xs leading-6 text-green-900">
          {colors.map((item) => t(item.name)).join(", ")}
        </p>
        <Ball color={number.color as never} size={rem(28)}>
