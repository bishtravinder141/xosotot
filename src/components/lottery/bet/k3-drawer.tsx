...
type LotteryK3BetDrawerProps = PropsWithChildren<{
  action: (data: FormData) => Promise<{ message: string } | { error?: string | undefined } | undefined>;
}>;

export default function LotteryK3BetDrawer(props: LotteryK3BetDrawerProps) {
  ...
  useEffect(() => {
    if (k3Bets.length > 0) {
      const normal = k3Bets.filter((item) => !["8", "10"].includes(item.type));
      const group2 = k3Bets.filter((item) => item.type === "8");
      const group3 = k3Bets.filter((item) => item.type === "10");

      const value = normal.length + combinations(group2, 2) + combinations(group3, 3);

      setPrice(amount * count * value);
    }
  }, [amount, count, k3Bets]);
    ...
    <Portal>
      <div
        className={twMerge(
          ...
          k3Bets.length < 1 && "translate-y-full",
        )}
      >
