...
type VietnamLotteryDavDvoiPageProps = {
  searchParams: {
    area?: string;
    date?: string;
    page?: string;
...
export default async function VietnamLotteryDavDvoiPage(props: VietnamLotteryDavDvoiPageProps) {
  ...
  return (
    <LotteryProvider>
      ...
      <LotteryTabsProvider initial="1">
        <TabsContent value="1">
          <LotteryRules>{t("lottery.vietnam-lottery.dav-dvoi.rules.0")}</LotteryRules>
        </TabsContent>
        <TabsContent value="2">
          <LotteryRules>{t("lottery.vietnam-lottery.dav-dvoi.rules.1")}</LotteryRules>
        </TabsContent>
        ...
        <Suspense fallback={<Skeleton className="h-14 rounded-xl" />}>
          <VietnamLotteryPreviousResult area={area} city={cityName} title="Đầu Đuôi" />
        </Suspense>
        ...
        <TabsContent value="1">
          <div className="flex flex-col rounded-xl border border-red-300 bg-red-50 p-4">
            <span className="mb-2.5 text-xs">
              Giải đặc biệt: x x x <mark className="bg-transparent font-bold text-red-300">?</mark> x
            </span>

            <div className="grid grid-cols-5 gap-x-3.5 gap-y-2.5">
              {Array.from(Array(10)).map((_, index) => (
                <LotteryBetTrigger
                  className="group relative aspect-square rounded-xl bg-red-300 text-white"
                ...
                >
                  ...
                  <span className="absolute left-1.5 top-1.5 inline-flex rounded-full p-0.5 ring-1 ring-white">
                    <span className="size-2 rounded-full bg-transparent transition-colors group-data-[state=active]:bg-white" />
                  </span>
                </LotteryBetTrigger>
              ))}
            </div>
        ...
        <TabsContent value="2">
          <div className="flex flex-col rounded-xl border border-red-300 bg-red-50 p-4">
            <span className="mb-2.5 text-xs">
              Giải đặc biệt: x x x x <mark className="bg-transparent font-bold text-red-300">?</mark>
            </span>
        ...
        <LotteryBetDrawer action={submit}>
          ...
          <input name="typeid" type="hidden" value="3" />
        </LotteryBetDrawer>
            ...
            <TabsContent value="history">
              <div className="min-w-max scroll-mt-20 space-y-2.5 overflow-x-auto">
                ...
                <Suspense fallback={<LotteryRecordsSkeleton limit={20} />}>
                  <LotteryVietnamLotteryHistory area={area} city={cityName} page={page} title="Đầu Đuôi" />
                </Suspense>
              </div>
            </TabsContent>
            ...
            <TabsContent value="bet history">
              ...
              <div className="min-w-max scroll-mt-20 space-y-2.5 overflow-x-auto">
                <Suspense fallback={<LotteryDetailsSkeleton limit={20} />}>
                  <LotteryVietnamLotteryDetails page={page} title="Đầu Đuôi" />
                </Suspense>
              </div>
            </TabsContent>
          </div>
