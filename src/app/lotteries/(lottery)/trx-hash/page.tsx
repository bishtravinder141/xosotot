import { submit } from "@action/lottery/trx-hash";
...
import LotteryTrxHashBet from "@components/lottery/bet/trx-hash";
import LotteryTrxHashResultPopup from "@components/lottery/bet/trx-hash-result-popup";
import LotteryTrxHashPeriod from "@components/lottery/period/trx-hash";
import LotteryTrxHashResult from "@components/lottery/result/trx-hash";
import LotteryTrxHashResultSkeleton from "@components/lottery/result/trx-hash.skeleton";
...
type LotteryTrxHashPageProps = {
  searchParams: {
    period?: string;
  };
};

export default async function LotteryTrxHashPage(props: LotteryTrxHashPageProps) {
  ...
  return (
    <>
      <Suspense>
        <LotteryTrxHashResultPopup type={period.type} />
      </Suspense>
      ...
      <LotteryRules>{t("lottery.trx-hash.rules")}</LotteryRules>
      ...
      <div className="flex flex-col items-center gap-2 rounded-lg bg-red-300 p-2.5 text-white">
        <div className="flex gap-2 text-[0.625rem]">
          ...
          <Suspense fallback={<Skeleton className="my-0.5 h-3 w-20 rounded" />}>
            <LotteryTrxHashPeriod type={period.type} />
          </Suspense>
        </div>
      ...
      <div className="flex justify-between rounded-lg bg-red-50 p-5">
        <Suspense fallback={<LotteryTrxHashResultSkeleton />}>
          <LotteryTrxHashResult type={period.type} />
        </Suspense>
      </div>
