...
import LotteryTrxHashDetails from "@components/lottery/details/trx-hash";
import LotteryRecordsSkeleton from "@components/lottery/records/records.skeleton";
import LotteryTrxHashChart from "@components/lottery/records/trx-hash-chart";
import LotteryTrxHashHistory from "@components/lottery/records/trx-hash-history";
      ...
      <div className="-mx-5 space-y-5 overflow-x-auto px-5">
        <TabsContent value="history">
          <div className="min-w-max scroll-mt-20 space-y-2.5 overflow-x-auto">
            <header className="flex gap-4 rounded-lg bg-blue-100 px-4 py-3 text-center text-[0.625rem] font-bold leading-4">
              ...
              <span className="w-20 grow">{t("Block Height")}</span>
              <span className="w-20 grow">{t("Hash Value")}</span>
              <span className="w-12 grow">{t("Result")}</span>
            </header>

            <Suspense fallback={<LotteryRecordsSkeleton limit={20} />} key={key}>
              <LotteryTrxHashHistory page={page} type={period.type} />
            </Suspense>
          </div>
        </TabsContent>
        ...
        <TabsContent value="chart">
          <div className="min-w-max scroll-mt-20 space-y-2.5 overflow-x-auto">
            ...
            <Suspense fallback={<LotteryRecordsSkeleton limit={20} />} key={key}>
              <LotteryTrxHashChart page={page} type={period.type} />
            </Suspense>
          </div>
        </TabsContent>
        ...
        <TabsContent value="bet history">
          ...
          <div className="min-w-max scroll-mt-20 space-y-2.5 overflow-x-auto">
            <Suspense fallback={<LotteryDetailsSkeleton limit={20} />} key={key}>
              <LotteryTrxHashDetails page={page} type={period.type} />
            </Suspense>
          </div>
        </TabsContent>
      </div>
